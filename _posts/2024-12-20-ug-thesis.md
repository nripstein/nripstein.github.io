...
<ol>
    <li>
        ...
        <ul>
            ...
            <li>Combined these frame-wise predictions with time-series segmentation; Initial results were promising, but the image classifier faced challenges generalizing to unseen participants.</li>
        </ul>
    </li>
    <li>
...
A selective review of offline change point detection highlights that the Pruned Exact Linear Time (PELT) algorithm finds the optimal solution to the segmentation problem with unknown numbers of changepoints {% cite truong2020selective killick2012optimal --file computer-vision-references %}. The [ruptures](https://centre-borelli.github.io/ruptures-docs/) Python package provides an easy-to-use implementation of PELT and related time series segmentation algorithms.

<div class="text-center mt-4">
    <img src="/assets/img/pelt.png" alt="PELT" class="img-fluid rounded" style="max-width: 100%; height: auto;" />
    <figcaption>Figure 5: PELT algorithm for time series segmentation (reproduced from {% cite truong2020selective --file computer-vision-references %})</figcaption>
</div>
<br>
...
A hand-object interaction dataset with 100,000 annotated images provides bounding boxes around hands and the objects with which
they are in contact {% cite shan2020hands --file computer-vision-references %}. The model distinguishes between stationary objects (e.g. tables and furniture) and
portable objects which can be moved by hands, and it is built on the Faster R-CNN object detection network {% cite ren2015faster --file computer-vision-references %}, which obtains strong
results.

I applied this object detection model {% cite shan2020hands --file computer-vision-references %} on frames extracted
...
I also briefly tried an egocentric hand-object segmentation model {% cite zhang2022egohos --file computer-vision-references %} (which is shown in the video at the top of this page), although it had worse performance than the object detection model I ultimately used {% cite shan2020hands --file computer-vision-references %}. My application doesn't require the fine-grained pixel-wise predictions of image segmentation, so I stuck with the more accurate image classifier.
...
In the temporal action segmentation literature, “timestamp supervision” refers to a specific type of semi-supervised learning, where most training data is unlabeled, but in each video, there is (at least) one frame with a class label and the label’s associated timestamp {% cite li2021timestamp ding2024temporal --file computer-vision-references %}.
...
<ol>
    <li>Image classifier-based method using I3D for temporally-sensitive feature extraction {% cite carreira2017quo --file computer-vision-references %}.</li>
    ...
    <li>Retrain the hand-object interaction model I used {% cite shan2020hands --file computer-vision-references %} but use feature extraction from I3D. This would likely help because that work was focused on object detection in single images, rather than sequences.</li>
    <li>
        Snorkel {% cite ratner2017snorkel --file computer-vision-references %} could be used in conjunction with my proposed methods for automatically generating training data for use with specialized models.
        <ul>
...
{% bibliography --group_by none --cited_in_order --file computer-vision-references %}
</div>
